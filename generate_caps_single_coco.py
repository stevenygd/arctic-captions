...
def main(model, saveto, k=5, normalize=False, zero_pad=False, datasets='dev,test',
         sampling=False, pkl_name=None, batch_id=1):
    ...
    train, valid, test, worddict = load_data(path='./data/coco/',
    ...
    def _process_examples(contexts):
        ttl = contexts.shape[0]
        if batch_id > 0 and batch_id * 500 < ttl:
            contexts = contexts[(batch_id-1)*500:batch_id*500]
        else:
            contexts = contexts[(ttl/500)*500:]
    ...
    for dd in ds:
        if dd == 'train':
            ...
            with open(saveto+'batch%d.train.txt'%batch_id, 'w') as f:
                print >>f, '\n'.join(caps)
            print 'Done'
        if dd == 'dev':
            ...
            # import pdb; pdb.set_trace()
            with open(saveto+'batch%d.dev.txt'%batch_id, 'w') as f:
                print >>f, '\n'.join(caps)
            print 'Done'
        if dd == 'test':
            print 'Test Set...',
            caps = _seqs2words(_process_examples(test[1]))
            with open(saveto+'batch%d.test.txt'%batch_id, 'w') as f:
                print >>f, '\n'.join(caps)
            print 'Done'
...
if __name__ == "__main__":
    ...
    main(args.model, args.saveto, k=args.k, zero_pad=args.z, pkl_name=args.pkl_name, normalize=args.n, datasets=args.d, sampling=args.sampling, batch_id=args.b)
